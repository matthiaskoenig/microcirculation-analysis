...
def stabilize_video(video_in: Path, video_out: Path):
    ...
    fig1.savefig(video_in.parent / f"{video_in.stem}_trajectory.png", bbox_inches="tight")
    plt.show()

    fig2, (ax3, ax4) = stabilizer.plot_transforms()
    fig2.savefig(video_in.parent / f"{video_in.stem}_transforms.png", bbox_inches="tight")
    plt.show()
...
if __name__ == "__main__":
    ...
    for video_in in sorted(input_dir.glob('*.avi')):
        if "converted" in str(video_in):
            print(video_in)
            video_stable = video_in.parent / f"{video_in.stem}_stable.avi"
